...
import com.moko.hyprosgw.databinding.ActivityScanTimeoutBinding;
...
public class ScanTimeoutActivity extends BaseActivity<ActivityScanTimeoutBinding> {
    ...
    private void setScanTimeout(int timeout) {
        ...
        try {
            MQTTSupport.getInstance().publish(getTopic(), message, MQTTConstants.CONFIG_MSG_ID_BLE_SCAN_TIMEOUT, appMqttConfig.qos);
    ...
    private void getScanTimeout() {
        ...
        try {
            MQTTSupport.getInstance().publish(getTopic(), message, MQTTConstants.READ_MSG_ID_BLE_SCAN_TIMEOUT, appMqttConfig.qos);
        } catch (MqttException e) {
            e.printStackTrace();
        }
    }
