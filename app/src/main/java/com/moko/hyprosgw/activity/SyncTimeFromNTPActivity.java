...
import com.moko.hyprosgw.databinding.ActivitySyncFromNtpBinding;
...
public class SyncTimeFromNTPActivity extends BaseActivity<ActivitySyncFromNtpBinding> {
    ...
    private void getNtpServer() {
        ...
        try {
            MQTTSupport.getInstance().publish(getTopic(), message, MQTTConstants.READ_MSG_ID_NTP_SERVER, appMqttConfig.qos);
    ...
    private void saveParams() {
        ...
        try {
            MQTTSupport.getInstance().publish(getTopic(), message, MQTTConstants.CONFIG_MSG_ID_NTP_SERVER, appMqttConfig.qos);
        } catch (MqttException e) {
            e.printStackTrace();
        }
    }
