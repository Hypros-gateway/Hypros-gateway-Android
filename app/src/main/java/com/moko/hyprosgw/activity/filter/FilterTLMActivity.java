...
public class FilterTLMActivity extends BaseActivity<ActivityFilterTlmBinding> {
    ...
    private void getFilterTlm() {
        ...
        try {
            MQTTSupport.getInstance().publish(getTopic(), message, MQTTConstants.READ_MSG_ID_FILTER_TLM, appMqttConfig.qos);
    ...
    private void saveParams() {
        ...
        try {
            MQTTSupport.getInstance().publish(getTopic(), message, MQTTConstants.CONFIG_MSG_ID_FILTER_TLM, appMqttConfig.qos);
        } catch (MqttException e) {
            e.printStackTrace();
        }
    }
