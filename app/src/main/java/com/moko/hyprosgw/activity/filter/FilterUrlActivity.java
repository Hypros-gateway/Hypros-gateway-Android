...
public class FilterUrlActivity extends BaseActivity<ActivityFilterUrlBinding> {
    ...
    private void getFilterUrl() {
        ...
        try {
            MQTTSupport.getInstance().publish(getTopic(), message, MQTTConstants.READ_MSG_ID_FILTER_URL, appMqttConfig.qos);
    ...
    private void saveParams() {
        ...
        try {
            MQTTSupport.getInstance().publish(getTopic(), message, MQTTConstants.CONFIG_MSG_ID_FILTER_URL, appMqttConfig.qos);
        } catch (MqttException e) {
            e.printStackTrace();
        }
    }
